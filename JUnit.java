...
/**
 * 
 * JUnit testing all of the methods that are part of the game Some game classes
 * are not included due to networking. They are tested on the outside inside the
 * log
 *
 * @author Eric Zhang
 * @version May 30, 2019
...
public class JUnit
{
    ...
    public void plusFourDoActionSecondCase()
    {
        ...
        {
            assertEquals( game.getPlayers()[p.getIdNum() - 1].getHand().size(),
                game.getPlayers()[p.getIdNum() + 1].getHand().size() + 4 );
            assertEquals( game.getColor(), "Yellow" );
            x--;
            if ( x == -1 )
            {
